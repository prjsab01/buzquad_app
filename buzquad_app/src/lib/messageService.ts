...
export async function sendMessage(conversationId: string, senderUid: string, text: string, imageUrl?: string) {
  const conversationRef = doc(firestore, 'conversations', conversationId);
  ...
  // Strip HTML tags for inbox preview
  const plainText = text.replace(/<[^>]+>/g, '').trim();
  const preview = imageUrl ? '📷 Photo' : (plainText.length > 60 ? plainText.slice(0, 60) + '…' : plainText);
