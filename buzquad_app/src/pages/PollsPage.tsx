    ...
    <section className="mx-auto max-w-5xl p-6">
      <div className="space-y-6 card p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-semibold text-app">Polls</h2>
            <p className="mt-1 text-app-2">Vote on community polls or create your own.</p>
          </div>
          {user && (
            <button
          ...
          <form className="space-y-3 rounded-xl border border-app bg-subtle p-4" onSubmit={handleCreate}>
            ...
            {type !== 'yesno' && (
              <div className="space-y-2">
                ...
                <button
                  type="button"
                  className="text-xs text-app-3 underline"
        ...
        {polls.length === 0 ? (
          <p className="text-app-3">No polls yet. Create one!</p>
        ) : (
            ...
            {polls.map((poll) => {
              ...
              const options =
                poll.type === 'yesno'
                  ...
                  : poll.options ?? [];
              return (
                <article key={poll.id} className="rounded-2xl border border-app bg-subtle p-5">
                  <h3 className="font-semibold text-app">{poll.question}</h3>
                  <p className="text-xs text-app-3">by {poll.authorDisplayName} · {poll.totalVotes} vote{poll.totalVotes !== 1 ? 's' : ''}</p>
                    ...
                    {options.map((opt) => {
                      ...
                      return (
                        <button
                          ...
                          className="relative w-full overflow-hidden rounded-lg border border-app px-3 py-2 text-left text-sm disabled:cursor-default"
                        >
                          <span
                          ...
                          <span className="relative flex justify-between">
                            <span>{opt.text}</span>
                            {hasVoted && <span className="text-app-3">{pct}%</span>}
                          </span>
                        </button>
                      );
                    })}
