      ...
      <div className="space-y-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-slate-600">Upcoming events, sessions, and meetups.</p>
          </div>
          {user && (
            <button
        ...
        {events.length === 0 ? (
          <p className="text-slate-500">No upcoming events. Create one!</p>
        ) : (
          <div className="space-y-4">
            {events.map((ev) => (
              <article key={ev.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="font-semibold text-slate-900">{ev.title}</h3>
                    <p className="text-xs text-slate-500 capitalize">{ev.type} · {formatDate(ev.startAt)}</p>
                  </div>
                  <span className="text-xs text-slate-400">{ev.rsvpCount} RSVP{ev.rsvpCount !== 1 ? 's' : ''}</span>
                </div>
                {ev.description && <p className="mt-2 text-sm text-slate-600">{ev.description}</p>}
                <p className="mt-1 text-xs text-slate-400">Hosted by {ev.hostDisplayName}</p>
                {user && (
                  <button
