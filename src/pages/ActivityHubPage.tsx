...
function ItemCard({
  ...
  return (
    <article className="flex flex-col rounded-2xl border border-slate-200 bg-slate-50 overflow-hidden">
      {item.thumbnailUrl && (
        <img
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        {item.tags && item.tags.length > 0 && (
          <div className="flex flex-wrap gap-1">
            {item.tags.map((t) => (
              <span key={t} className="rounded-full bg-slate-200 px-2 py-0.5 text-xs text-slate-600">
                {t}
              </span>
            ))}
          </div>
  ...
  return (
    <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-3 gap-3">
      <div className="min-w-0">
        <p className="truncate font-medium text-slate-900 text-sm">{session.itemTitle}</p>
      ...
      <div className="flex shrink-0 gap-2">
        ...
        {isParticipant && !isHost && (
          <span className="rounded-lg bg-slate-100 px-3 py-1.5 text-xs text-slate-500">Joined</span>
        )}
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-slate-900">Start Session</h3>
          <button onClick={onClose}><X size={18} /></button>
        </div>
        <p className="text-sm text-slate-600 mb-4 truncate">{item.title}</p>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-slate-900">Add {category} item</h3>
          <button onClick={onClose}><X size={18} /></button>
        </div>
    ...
    <section className="mx-auto max-w-6xl p-6">
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-5">
          <div>
            <h2 className="text-3xl font-semibold text-slate-900">Activity Hub</h2>
            <p className="mt-1 text-slate-600 text-sm">Browse, start, or join shared activity sessions.</p>
          </div>
          {isAdmin && (
            <button
